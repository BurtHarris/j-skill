...
async function importFromGitHub(
  ...
  try {
    ...
    diagnostics.error(
      `failed to fetch ${owner}/${repo}: ${(e as Error).message}`,
    );
    return null;
  }

  const hasCommands = topLevel.some(
    (e) => e.name === "commands" && e.type === "dir",
  );
  const hasSkills = topLevel.some(
    (e) => e.name === "skills" && e.type === "dir",
  );

  if (!hasCommands && !hasSkills) {
    diagnostics.warn(
      `${owner}/${repo}: no 'commands/' or 'skills/' directory found`,
    );
  }
  ...
  if (hasCommands) {
    ...
    for (const entry of commandEntries) {
      ...
      try {
        ...
        diagnostics.error(
          `failed to fetch ${entry.path}: ${(e as Error).message}`,
        );
        continue;
      }
      ...
      if (!hasFrontmatter) {
        diagnostics.warn(
          `${entry.path}: no YAML frontmatter found, skipping`,
        );
        continue;
      }
      ...
      Deno.writeTextFileSync(join(getCommandsDir(), entry.name), content);
  ...
  if (hasSkills) {
    ...
    for (const dir of skillDirs) {
      ...
      if (!hasFrontmatter) {
        diagnostics.warn(
          `${skillMdPath}: no YAML frontmatter found, skipping`,
        );
        continue;
      }
      ...
      try {
        skillFiles = await fetchGitHubContents(
          owner,
          repo,
          `skills/${dir.name}`,
        );
      ...
      for (const f of skillFiles) {
        ...
        Deno.writeTextFileSync(join(destDir, f.name), fileContent);
...
export async function runImport(source: string): Promise<void> {
  ...
  if (!parsed) {
    diagnostics.error(
      `cannot parse source: '${source}'. Use owner/repo, https://github.com/owner/repo, or a local path`,
    );
    diagnostics.report();
    Deno.exitCode = 1;
    return;
  ...
  Deno.mkdirSync(getRegistryDir(), { recursive: true });
  Deno.mkdirSync(getManifestsDir(), { recursive: true });
  ...
  if (diagnostics.hasErrors()) {
    console.error(
      `import failed with ${diagnostics.errors.length} error(s)`,
    );
    Deno.exitCode = 1;
    return;
  }
